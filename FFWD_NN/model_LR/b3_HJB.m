...
V0     = zeros(nval_a, nval_z, nval_B, nval_N);
V0_n   = zeros(nval_a, nval_z, nval_B, nval_N, maxitHJB); % this one takes 200MB of RAM - I still can afford it

u_stacked  = zeros(nval_a*nval_z*nval_B*nval_N,1);
V0_stacked = zeros(nval_a*nval_z*nval_B*nval_N,1);
...
for itHJB = 1:maxitHJB
    ...
    V0_n(:,:,:,:,itHJB) = V0;              % keeps track of the value function
    ...
    for iN=1:nval_N    % reshape could be faster, but this is clearer
        for iB=1:nval_B
            for iz=1:nval_z
                u_stacked(1+(iz-1)*nval_a+(iB-1)*nval_a*nval_z+(iN-1)*nval_a*nval_z*nval_B:nval_a+(iz-1)*nval_a+(iB-1)*nval_a*nval_z+(iN-1)*nval_a*nval_z*nval_B,1)=u(:,iz,iB,iN);
            end
        end
    end
    ...
    for iN=1:nval_N    % reshape could be faster, but this is clearer
        for iB=1:nval_B
            for iz=1:nval_z
                V1(:,iz,iB,iN)=V1_stacked(1+(iz-1)*nval_a+(iB-1)*nval_a*nval_z+(iN-1)*nval_a*nval_z*nval_B:nval_a+(iz-1)*nval_a+(iB-1)*nval_a*nval_z+(iN-1)*nval_a*nval_z*nval_B,1);
            end
        end
    end
    ...
    % CHECK CONVERGENCE
    dist(itHJB) = max(max(max(max(abs(Vchange)))));
    disp(dist(itHJB))
...
for iN=2:nval_N-1    % reshape could be faster, but this is clearer
    for iB=2:nval_B-1
        for iz=1:nval_z
