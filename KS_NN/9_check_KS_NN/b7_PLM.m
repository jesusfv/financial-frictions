...
PLM_IP_finegrid = zeros(nval_KK,nval_Z)*NaN;
...
for iK=1:nval_KK
    for iZ=1:nval_Z
        ...
        for it_sim=1:size(Y,1)
            if abs(X1(it_sim)-KK_grid(iK))<dKK/2
                if abs(X2(it_sim)-Z_grid(iZ))<dZ/2          % If the simulation falls in a particular grid point
                    Y_=[Y_;Y(it_sim)];
                    X1_=[X1_;X1(it_sim)];
                    X2_=[X2_;X2(it_sim)];
                end
        ...
        if size(X1_,1)>5
            ...
            X = [X0_ X1_ X2_];
            beta = (X'*X)^-1*X'*Y_;
            PLM_IP_finegrid(iK,iZ) = [1 KK_grid(iK) Z_grid(iZ)]*beta;  % We evaluate only the grid point
        end
    end
end
...
for iK=1:nval_KK
    for iZ=1:nval_Z
        if not(isnan(PLM_IP_finegrid(iK,iZ)))
            Y_=[Y_;PLM_IP_finegrid(iK,iZ)];
            X1_=[X1_;KK_grid_2D(iK,iZ)];
            X2_=[X2_;Z_grid_2D(iK,iZ)];
        end
    end
end
